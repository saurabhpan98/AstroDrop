...
import { KeyRound, Send, ArrowRight, Compass, Wifi, Globe } from 'lucide-react';

export default function DiscoveryPanel({ nearbyPeers, onConnectNearby, onConnectCode, selfCode }) {
  const [remoteCode, setRemoteCode] = useState('');
  const [activeTab, setActiveTab] = useState('orbit'); // 'orbit' | 'warp'
                ...
                nearbyPeers.map((peer) => (
                  ...
                  >
                    <div className="flex items-center space-x-2.5 sm:space-x-3 truncate">
                      <span className="text-xl sm:text-2xl">{peer.profile?.avatar?.split(' ')[0] || '🪐'}</span>
